...
<body class="bg-light">
	...
	<section class="container text-center mb-4">
		<h1>
			...
			,<br> voici quelques détails du produit !
		</h1>
	</section>
	...
	<section class="container border bg-white p-4 mb-4 w-50">
		...
		<p>
			...
			<c:out value="${requestScope.articledesciption}" />
		<p>
		<p>
			<strong>Meilleure offre :</strong>
			<c:out value="${requestScope.encherefaite}" />
			points, par
			<c:out value="${requestScope.articlebuyerpseudo}" />
		</p>

		<p>
			<strong>Mise à prix :</strong>
			<c:out value="${requestScope.articlesStartPrice}" />
			points
		</p>
		<p>
			<strong>Fin de l'enchère :</strong>
			<c:out value="${requestScope.articlefinenchere}" />
		</p>
		<p>
			<strong>Retrait :</strong>
			<c:out value="${requestScope.articleretraitstreet}" />
			,
			<c:out value="${requestScope.articleretraitpostal}" />
			<c:out value="${requestScope.articleretraitCity}" />
		</p>
		<p>
			<strong>Vendeur :</strong>
			<c:out value="${requestScope.articleretraitpseudo}" />
		</p>
	...
	<section class="container border bg-white p-5 w-50 ">
		<form action="<c:url value = "/achatArticle"/>" method="post">
			...
			<div class="mb-3 form-group">
				...
				<p>
					Proposition d'enchère (Meilleure offre :
					<c:out value="${requestScope.encherefaite}" />
					points)
				</p>
